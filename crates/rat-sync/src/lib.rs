...
fn collect_comment_tags(
    ...
    walk_nodes(tree.root_node(), &mut |node| {
        ...
        let cap = first.expect("tag capture exists");
        let raw_id = cap.get(1).expect("id capture").as_str();
        let packet_type = cap.get(2).expect("type capture").as_str().to_string();
        let packet_id =
        ...
        let matched = cap.get(0).expect("whole capture");
        tags.push(TagMatch {
